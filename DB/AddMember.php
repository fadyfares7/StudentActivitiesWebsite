		  ...
		  <h2 class="sub-header">Add Member</h2>
			...
			?>
              <div class="panel panel-default col-sm-12" id="rcorners1">
                ...
                <input type="hidden" name="portal" value="add_member"/>
            ...
            <select class="form-control" id="com" name="com" required>
              <?php
                $query  = "SELECT `committee`.`Com_ID`,`Name` ";
                $query .= "FROM `committee`,`sa_committee` ";
                $query .= "WHERE `committee`.`Com_ID`=`sa_committee`.`Com_ID` AND `SA_ID`=".$_SESSION['SA_admin'];
